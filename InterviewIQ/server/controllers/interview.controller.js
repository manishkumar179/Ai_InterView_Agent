...
// For analyze resume and extract data from it
export const analyzeResume = async (req, res) => {
  try {
    ...
    const messages = [
      {
        role: "system",
        content: `Extract structured data fro resume.
...
// Controller for Create question
export const generateQuestion = async (req, res) => {
  try {
    ...
    if (user.credits < 50) {
      return res.status(400).json({
        message: "Not enough credits. Minimum 50 credites required.",
      });
    }
...
// Controller for submit answer and to score these questions
...
// controller for current user final report
export const getMyInterviews = async (req, res) => {
  try {
    const interview = await Interview.find({ userId: req.userId })
      .sort({ createdAt: -1 })
      .select("role experience mode finalScore status ceatedAt");
